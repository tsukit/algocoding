...
# SOLUTION
# ========
# this is an implementation of minimum path sum. the way it works is based on a simple observation:
#
# 	current_min_path_sum = current_value + min(left_min_path_sum, above_min_path_sum)
#
# we use dynamic programming technique to avoid computing the same value multiple times. time
# complexity of this implementation is O(row x col)
def min_path_sum(grid):
	# memory for dynamic programming. this saves us from compiting the sum that has already been computed
	memory = [[(None, None)] * len(r) for r in grid]
	import math

	# this is the core of the implementation. to find the minimum sum of the current cell, we choose the
	# smaller of minimum sum the left one or the one above it
	def min_sum_for(row, col):
		...
		# if we have to compute it, see if its the top-left cell of the grid. if it is, it's just
		# the value of the cell itself
		if row == 0 and col == 0:
			memory[row][col] = grid[row][col], [(row, col)]
			return memory[row][col]

		# otherwise, get the minimum sum of the left and above one. we then choose the smaller one
		# and add the current value of on top of it
...
def print_grid(grid, indent='\t'):
	for row in range(len(grid)):
		line = ','.join([f'{grid[row][col]:5}' for col in range(len(grid[row]))])
		print(f'{indent}{line}')
